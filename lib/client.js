...
/* ── 悬浮窗外壳 ── */
.dsh-pm-win {
  position: absolute; z-index: 60; width: ${WIN_W}px; height: ${WIN_H}px;
...
.dsh-pm-win[data-collapsed] { height: auto; }
...
/* 主体 */
.dsh-pm-body { flex: 1; min-height: 0; overflow-y: auto; padding: 10px 12px 12px; display: flex; flex-direction: column; gap: 10px; }
.dsh-pm-empty { padding: 26px 14px; border: 1px dashed var(--dsw-alias-border-l2, #ccc); border-radius: 12px; text-align: center; font-size: 12px; color: var(--dsw-alias-label-tertiary, #999); line-height: 1.7; }
.dsh-pm-error { font-size: 12px; color: #dc2626; padding: 4px 0; }
...
`;
    ...
    function FloatingWindow() {
      ...
      useEffect(() => {
        const onResize = () => {
          ...
          const next = {
            ...
            y: Math.min(Math.max(0, posRef.current.y), Math.max(0, h - 60)),
          };
      ...
      const onTitlePointerMove = (e) => {
        ...
        const y = Math.min(Math.max(0, e.clientY - drag.current.dy), h - 40);
        posRef.current = { x, y };
